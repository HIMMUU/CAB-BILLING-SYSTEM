...
export class ReportsService {
  ...
  async getOutstandingReport() {
    ...
    for (const inv of outstandingInvoices) {
      ...
      const diffDays = Math.floor(
        (today.getTime() - dueDate.getTime()) / oneDayMs,
      );
          ...
          trip: {
            booking: {
              ...
              employeeId: query.employeeId
                ...
                : undefined,
              pickupDate:
                query.dutyDateFrom || query.dutyDateTo
                  ? {
  ...
  async generateBillRegisterPdf(query: {
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      data.forEach((row, index) => {
        ...
        // Draw Basic Amt
        doc.text(
          row.basicAmt.toLocaleString('en-IN', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }),
          455 + 3,
          currentY + 4,
          { width: 65 - 6, align: 'right' },
        );

        // Draw P/T/Taxes
        doc.text(
          row.ptTaxes.toLocaleString('en-IN', {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }),
          520 + 3,
      ...
      doc.text(
        grandBasicAmt.toLocaleString('en-IN', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }),
        455 + 3,
      ...
      doc.text(
        grandPtTaxes.toLocaleString('en-IN', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }),
        520 + 3,
      ...
      doc.text(
        grandIgst.toLocaleString('en-IN', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }),
        585 + 3,
      ...
      doc.text(
        grandCgst.toLocaleString('en-IN', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }),
        640 + 3,
      ...
      doc.text(
        grandSgst.toLocaleString('en-IN', {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        }),
        695 + 3,
  ...
  async getDutySlipRegisterData(query: {
    ...
    return slips.map((slip, idx) => {
      ...
      const clientName = slip.booking?.customer?.name || (slip as any).manualCustomerName || 'Direct Customer';
      const guestName = (slip as any).guestName || slip.booking?.guestName || '—';
      const driverName = slip.driver?.name || (slip as any).manualDriverName || 'Manual Driver';
      const vehicleNo = slip.vehicle?.vehicleNumber || (slip as any).manualVehicleNumber || 'Manual Vehicle';
  ...
  async generateDutySlipRegisterPdf(query: {
    ...
    return new Promise<Buffer>((resolve, reject) => {
      ...
      data.forEach((row) => {
        ...
        const rowHeight =
          Math.max(18, clientNameHeight, guestNameHeight, driverNameHeight) + 6;
