...
let DutySlipsService = class DutySlipsService {
    ...
    async generatePdf(id) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const statusColor = slip.status === 'CLOSED'
                ? '#16A34A'
                : slip.status === 'FILLED'
                    ? '#2563EB'
                    : '#D97706';
            doc
                ...
                .text(slip.status, 400, tableStartY + 33);
            ...
            const rawGuest = (slip.guestName ||
                slip.booking?.guestName ||
                slip.manualGuestName ||
                '').trim();
            const guestSal = (slip.guestSalutation ||
            ...
            const pickupLoc = slip.booking?.pickupLocation ||
                slip.manualPickupLocation ||
                '---';
            ...
            let driverRemarksVal = (slip.remarks ||
            ...
            doc
                .font(fontRegular)
                .text(driverRemarksVal
                ? driverRemarksVal.slice(0, 25)
                : '____________________', 380, sec4BoxY + 9, {
                width: 155,
                height: 14,
                lineBreak: false,
                ellipsis: true,
            ...
            doc
                ...
                .text(driverRemarksVal
                ? driverRemarksVal
                : '__________________________________________________________', 150, sec4BoxY + 27, {
                width: 380,
                height: 14,
                lineBreak: false,
                ellipsis: true,
            ...
            const sDateDisplay = sDateVal
                ...
                : '___/___/___  ___:___';
            const eDateVal = slip.endDateTime || slip.trip?.endDateTime;
            const eDateDisplay = eDateVal
                ? `${new Date(eDateVal).toLocaleDateString('en-GB')} ${new Date(eDateVal).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', hour12: false })}`
                : '___/___/___  ___:___';
            doc.fontSize(8.5).font(fontBold);
            doc.text(startKmDisplay, 55, sec5BoxY + 45);
            doc.text(endKmDisplay, 170, sec5BoxY + 45);
            doc.text(sDateDisplay, 285, sec5BoxY + 45);
            doc.text(eDateDisplay, 400, sec5BoxY + 45);
            ...
            doc
                ...
                ? 'INR ___________'
                : `INR ${Number(slip.toll || 0).toFixed(2)}`, 180, sec6BoxY + 10);
            doc.font(fontBold).text('Parking Charges:', 307, sec6BoxY + 10);
            doc
                .font(fontRegular)
                .text(!isClosedOrFilled
                ? 'INR ___________'
                : `INR ${Number(slip.parking || 0).toFixed(2)}`, 420, sec6BoxY + 10);
            doc.font(fontBold).text('State Tax:', 60, sec6BoxY + 40);
            doc
                .font(fontRegular)
                .text(!isClosedOrFilled
                ? 'INR ___________'
                : `INR ${Number(slip.stateTax || 0).toFixed(2)}`, 180, sec6BoxY + 40);
            doc.font(fontBold).text('MCD Toll:', 307, sec6BoxY + 40);
            doc
                .font(fontRegular)
                .text(!isClosedOrFilled
                ? 'INR ___________'
                : `INR ${Number(slip.mcd || 0).toFixed(2)}`, 420, sec6BoxY + 40);
            doc.font(fontBold).text('Night Allowance:', 60, sec6BoxY + 70);
            doc
                .font(fontRegular)
                .text(!isClosedOrFilled
                ? 'INR ___________'
                : `INR ${Number(slip.nightCharges || 0).toFixed(2)}`, 180, sec6BoxY + 70);
